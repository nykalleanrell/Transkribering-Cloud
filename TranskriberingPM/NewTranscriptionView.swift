...
struct NewTranscriptionView: View {
    ...
    @AppStorage("pm_model_size")  private var modelSize  = "small"
    ...
    var body: some View {
                ...
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if apiKey.isEmpty {
                        HStack(spacing: 8) {
                            ...
                            Text("OpenAI API-nyckel saknas — ange den i Inställningar.")
                                .font(.system(size: 12))
                        }
                        .padding(12)
                    ...
                    // Modellval
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Transkriptionsmodell").font(.system(size: 13, weight: .medium))
                        HStack(spacing: 10) {
                            ForEach([("medium", "Bra"), ("small", "Snabb")], id: \.0) { size, label in
                                Button(action: { modelSize = size }) {
                                    Text(label)
                                        .font(.system(size: 13, weight: modelSize == size ? .semibold : .regular))
                                        .padding(.horizontal, 16)
                                        .frame(height: 28)
                                        .background(modelSize == size ? Color.accentColor : Color(NSColor.controlBackgroundColor))
                                        .foregroundColor(modelSize == size ? .white : .primary)
                                        .cornerRadius(7)
                                }
                                .buttonStyle(.plain)
                            }
        ...
        .onAppear {
            if let u = initialURL { selectedURL = u }
            whisper.serverManager = serverManager
        }
    }
